+++
date = '2021-03-03'
title = 'Setting up a Blog - Part 2'
+++
...
the [website's source code at Github](https://github.com/TheVaffel/nocturne.no). The latest
commit at the time of writing, and thus probably the most relevant to what's written here,
is [d53fdd](https://github.com/TheVaffel/nocturne.no/tree/d53fddec77a1ea823dea51c741ea3f3de7cd8dc2).
...
Conviniently enough, React has something called `react-router` that does some of this for us.
However using `react-router` will give us <i>front-end routing</i> as opposed to <i>back-end routing</i>.
...
The `<Switch>` elements wrap the entire route structure. ReactRouter requires both the
...
<Db/>
The `<Link>` and `<Route>` tags are much what they sound like:
...
add `/route0` to the end of the URL (so you will see it change in the address bar), and make
...
The `exact` part inside the Route tags tells the Route's to only (surprise, surprise)
match if the URL path matches the given path exactly, and not just the prefix.

That was briefly how front-end routing works with `react-router`. Next up, we'll show
...
```
in place of our previous `filename`, which was `'main.bundle.js'`.
...
The `React.lazy` function turns the newly imported content to a component. It requires the
component to be the default export of the imported file.

The `Suspense` block makes sure the user has something to watch while they wait for the
...
First, we put all of the above into a nice handy-dandy function, `getLazy`:
```typescript
...
Extra care is taken in the render function for the very rare case of `render()`
getting called before the `setState()` call finishes (which might happen, from what I learned
tinkering around with this).

We have here used the name `_dcw_fileName` for the name of the file we want to
...
`/dynamic_path`, not a second before.
...
- show the user to help them decide what to read (or not to read)
- sort blog posts by newest to oldest
- show when the post was last updated
- know in what (.tsx) file the blog post can be found
...
#### Creating and reading the metadata
...
the `createdDate` and `updatedDate` to the current time and set the fileName.
...
the request to `/devblog/list` and uses the resulting list to show the user a selection of
...
```
inside the top-level devblog-route component. Here, `axios` is a random http-request
...
Here, we run a fancy `.map`-function over the meadata list and create a new `Route` component
...
we include a `key` in the `Route` component. This is necessary whenever React component
...
the top-level dev blog component. The `PostListEntry` component gives an entry like this:

```typescript
const PostListEntry: React.FunctionComponent<{metadata: Metadata}> = (props) => (
    <div>
        <Link to={devblogPath + '/' + getURLPart(props.metadata)}>
