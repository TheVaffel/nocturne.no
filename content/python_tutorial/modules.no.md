...
I denne posten skal vi først snakke litt overordnet om hva en modul er og helt konkret bruke `random`-modulen for å demonstrere hvordan vi jobber med moduler. Deretter går vi igjennom forskjellige vanlige moduler og viser hvordan vi kan bruke dem. Dette er bare et veldig beskjedent utvalg av alle modulene som finnes der ute.
...
Man har frihet til å legge slike `import`-linjer hvor man vil i programmet, men så godt som alle Python-programmer som finnes der ute legger alle `import`-linjer øverst i programmet, før all annen kode. Det gjør at det er lett å få oversikt over hvilke moduler programmet bruker.

La oss hoppe rett på et eksempel på en modul som brukes i mange Python-skript: `random`.


`random`-modulen inneholder forskjellige funksjoner for å lage tilfeldige tall. Alle programmene vi har sett på hittil har vært <i>deterministiske</i>, det vil si at programmet oppfører seg likt hver gang det kjøres hvis innputt er likt. Når vi introduserer tilfeldigheter, er ikke dette lenger tilfellet.

For å importere `random`-modulen i programmet vårt, skriver vi bare linja
```python
import random
```
Modulen er nå klar for å brukes.

I dette eksempelet lager vi bare et enkelt program for å simulere resultatet av å kaste en terning. Hver gang programmet startes, genereres det et tilfeldig tall mellom 1 og 6 som skrives ut til brukeren. Det kommer kanskje ikke som en overraskelse at `random`-modulen kan hjelpe oss med å lage det tilfeldige tallet, men hvordan vet vi hva `random` tilbyr, som vi kan bruke?

For å se hvilke funksjoner og andre ting en modul inneholder, er det best å slå opp i <i>dokumentasjonen</i> til modulen. Dokumentasjonen er som regel lett tilgjengelig på Internett. I vårt tilfelle vil et kjapt søk i en søkemotor med f. eks. <i>python random documentation</i> fortelle oss at dokumentasjonen for `random`-modulen befinner seg på  <a href="https://docs.python.org/3/library/random.html">https://docs.python.org/3/library/random.html</a>.
...
Dokumentasjonen for `random` starter med en overordnet forklaring av innholdet i modulen. Ikke få panikk om det er mange uforståelige ord i teksten, det er vanlig at dokumentasjon inneholder veldig spesifikk informasjon for avanserte brukere. For nybegynnere og generelt uinnvidde er det viktig å holde tunga rett i munnen og klare å lete seg fram til kun informasjonen vi vil ha og klarer å forstå.

Dokumentasjonen til `random` er delt opp i lett håndterbare seksjoner, og vi finner fort en som handler om heltall ("Functions for Integers"). Her har vi noen få funksjoner, og av dem er det `randint` som er mest interessant. `randint(a, b)` genererer et tilfeldig heltall innenfor intervallet mellom `a` og `b`, hvor begge endepunktene er med. Merk at Python er litt inkonsistent på dette punktet - mens en `range` ikke inkluderer sitt siste endepunktet, så er det siste endepunktet inkludert i `randint`.


Når vi omsider har funnet funksjonen vi vil bruke, er det bare å skrive det inn i programmet vårt. Vi har allerede importert `random`, og kan bruke `randint`-funksjonen i `random`-modulen ved å skrive
```python
...
import random
tilfeldig_tall = random.randint(1, 6)
print('Du kastet en terning og fikk tallet', tilfeldig_tall)
```
...
Dersom vi skulle brukt `randint` mange ganger, ville vi antakeligvis blitt lei av å skrive `random` foran hele tiden. I Python kan man importere modulinnhold som enkeltstående variabler med syntaksen
```python
from <modulnavn> import <funksjonsnavn1>, <funksjonsnavn2> ...
```
...
En annen måte å undersøke hvilke funksjoner en modul inneholder, er å kalle `dir()` på modulen. Altså ville kallet `dir(random)` (etter at modulen er importert) gitt deg en liste over alt innholdet i modulen. Dette er flott for å kjapt undersøke hva som tilbys, men er mangelfullt når det kommer til å si f. eks. hvilke argumenter en funksjon tar i bruk.

For å lese dokumentasjonen for en modul uten å søke på Internett, kan du også bruke Python i interaktiv modus og kjøre `help(<modulnavn>)` eller `help(<modulnavn>.<funksjonsnavn>)` etter å ha importert modulen.

### Modulen `math`

Vi nevner `math`-modulen litt kjapt i forbifarten for interesserte lesere: `math` inneholder alt mulig av vanlige matematiske operasjoner som for eksempel kvadratrot (`math.sqrt`), logaritmer (`math.log`, `math.log2`, `math.log10`) og trigonometriske funksjoner (`math.sin`, `math.cos` osv..). Dokumentasjonen for `math`-modulen kan du finne <a href="https://docs.python.org/3/library/math.html">her</a>.

### JSON-filer

JSON (<b>J</b>ava<b>s</b>cript <b>O</b>bject <b>N</b>otation) er et filformat som brukes for å lagre strukturert informasjon av én eller annen form. JSON-filer har filendelsen `.json`.
...
Mulig du allerede har lurt på hvorfor vi plutselig snakker om JSON. Det er fordi det er et vanlig format å bruke for å overføre informasjon på Internett, og fordi vi har en modul i Python som gjør lesing og tolking av en JSON-fil veldig lett, nemlig `json`-modulen.
...
For å demonstrere hvordan modulen fungerer, trenger vi JSON-filer å jobbe med. Legg de to JSON-eksemplene vi skrev over inn i to forskjellige filer. Hvis du bruker IDLE (eller en forholdsvis normal editor), kan du gjøre dette ved å lage en ny fil, markere og kopiere JSON-innholdet med Ctrl-C og lime det inn med Ctrl-V (eller tilsvarende på Mac). Når du lagrer filen, husk å velge "alle filtyper" og legge på `.json` på slutten av navnet. I våre eksempler kommer vi til å kalle de to filene henholdsvis `by.json` og `karakterer.json`.

La oss prøve å lese den første fila. Til dét kan vi bruke `load`-funksjonen i `json`-modulen. `load`-funksjonen tar inn et <i>filobjekt</i>, som betyr at vi må åpne fila vi vil lese, før vi sender den til `load`-funksjonen. Her er et eksempel der vi leser den første av de to filene:
```
import json
...
with open('by.json') as fil:
    ...
    print(bytabell['navn'], 'er en by i', bytabell['land'], 'med', \
        bytabell['befolkning'], 'mennesker')
```

(Bakoverskråstreken `\` har kun den funksjonen at den lar oss splitte et funksjonskall over flere linjer.)

Først importerer vi modulen, så åpner vi filen, og deretter lar vi `json.load`-funksjonen gjøre magien sin for å oversette filinnholdet til et Python-objekt. Her ser vi at vi kan bruke objektet direkte som en tabell etter å ha lest det ut.

Vi tar med et eksempel der vi først leser et objekt fra en JSON-fil, endrer objektet, og skriver det til en annen JSON-fil. Her bruker vi også `dump`-funksjonen, som gjør det omvendte av `load` - den skriver et Python-objekt til en fil.
```python
import json
...
```
Her har vi først lest ut karakterene, deretter går vi igjennom alle karakterene og passer på at Slur har en venn. Deretter åpner vi en ny fil for skriving, og bruker `dump` til å skrive lista med det oppdaterte objektet til filen.

`json` er vel og bra å kunne, fordi det forenkler prosessen med å lagre kompleks informasjon fra kjøringen av et program og lese det når programmet kjøres senere, for eksempel for å lagre noe brukeren har gjort. Men det er flere steder vi kan finne JSON-filer. Det ser vi på i neste seksjon:
...
For å sende forespørsler over Internett, kan vi bruke pakken `requests`. Men dersom du nå skriver `import requests` øverst i et program og kjører det, får du fort beskjed om at pakken ikke finnes. `requests` kommer nemlig ikke ferdig installert med de fleste Python-installasjoner, slik at du nå blir nødt til å installere den på maskinen din før vi kan bruke den. Vi går igjennom prosessen for installasjon her:
...
For å teste terminalen (uansett hvilket operativsystem du bruker), skriv `python3` på terminalen og trykk linjeskift. Hvis du får opp litt tekst og en `>>>`, som i den interaktive Python-konsollen, så kan du være sikker på at det fungerer, og du kan gå ut av Python-konsollen (skriv `exit()`), og fortsette på instruksjonene under. Hvis du derimot får beskjed om at programmet ikke ble funnet, tyder det på at Python ikke ble lagt til i `PATH`-variabelen ved installasjon. Dersom dette er tilfellet, vil den enkleste løsningen være å installere Python på nytt, og sørge for at du legger til Python i `PATH` som en del av installasjonen ved å følge instruksjonene i starten av denne bloggen. Ikke vær redd om du må gjøre dette - ingen av programmene du har skrevet hittil kommer til å forsvinne, så lenge du ikke har lagt dem akkurat der Python er installert!

<div class="notice">

I Windows risikerer du å oppleve et tredje alternativ: Windows åpner opp Microsoft Store for å laste ned Python. Dersom dette skjer, må du følge instruksjonene et stykke ned i Microsoft Store-vinduet; de ber deg om å gå til startmenyen og finne "Administrer appkjøringsaliaser" (engelsk: <b>Manage App Execution Aliases</b>), og der skru av aliaser for Python og Python3. Etter å ha gjort dét, kan du lukke terminalen du startet og åpne en ny en, og prøve å skrive `python3` på nytt.

</div>

##### `pip`

Den vanligste måten å installere pakker på i Python, er ved å bruke programmet `pip` som blir installert samtidig med Python. `pip` er et kommandolinjeprogram, som betyr at vi kjører det fra terminalen, og får ikke opp noe eget vindu med et brukergrensesnitt. Dersom du skriver `pip` i terminalen og trykker linjeskift, får du opp instruksjoner om hvordan programmet kan brukes. Ikke få panikk om du ikke skjønner bæret - vi går nå igjennom hvordan vi installerer en pakke:

Modulen vi skal bruke for web-forespørsler, heter som nevnt `requests`, og pakken den ligger i går under samme navn. For å installere den med `pip`, skriver vi rett og slett
```python
pip install requests
```
i terminalen og trykker enter. Dersom du er heldig, vil nå `pip` si at den laster ned `requests` og noen andre pakker. Årsaken til at den laster ned mer enn bare `requests`, er at `requests` er avhengig av disse pakkene for å kunne fungere. Dersom `pip` gir deg en feilmelding, er det beste håpet ditt å bruke feilmeldingen som søketekst på Internett og se om noen har et godt svar på problemet.

Dersom det foregående steget fungerte, gratulerer! `requests` er blitt installert! Kommandolinjeprogrammer som `pip` kan virke skumle i starten, men de er ekstremt effektive når du vet hvordan du bruker dem.
...
print('Meldingen er:', meldingsobjekt['melding'])
```

Vi har sendt en forespørsel, og med litt flaks, fått et svar fra serveren. Svaret vi får er et respons-objekt, som vi lagrer i variabelen `respons`. Responsvariabelen inneholder diverse informasjon i tillegg til selve responsdataen, men her er det bare dataen vi er interessert i. For denne forespørselen forventer vi å få en responsdata som inneholder et JSON-objekt med et felt som heter `melding`. For å hente ut dette bruker vi `.json()`-medlemsfunksjonen på responsobjektet, som automatisk oversetter JSON-innholdet til en Python-tabell. Til slutt skriver vi meldingen vi fikk ut på skjermen. Prøv koden over selv og se om du får ut meldingen!

La oss prøve å få tak i litt værdata ved hjelp av `requests`-modulen. For å få til det skal vi sende en forespørsel til <a href="met.no">met.no</a>, eid av Meteorologisk Institutt. I tillegg til en vanlig nettside, har met.no nemlig et <i>API</i> (<b>A</b>pplication <b>P</b>rogramming <b>I</b>nterface), som er et sett med stier man kan sende forespørsler til, beregnet for programmer i motsetning til menneskelige brukere.

Her er en oversikt over de forskjellige stiene vi kan bruke for å hente data fra met.no: <a href="https://api.met.no/weatherapi/locationforecast/2.0/#!/data/get_compact">https://api.met.no/weatherapi/locationforecast/2.0/#!/data/get_compact</a>. Slike stier kaller vi også <i>endepunkter</i>. Vi skal bruke endepunktet `/compact` i eksempelet vårt. Beskrivelsen kan være litt uoversiktelig i starten, men kort fortalt lar `/compact` oss sende inn koordinater for en hvilken som helst plass på jorda, og få tilbake værdata for dette stedet. Vi kommer til å bruke Oslo, som har koordinater 59.913889, 10.752222, som eksempel (koordinatene må være på desimalform, i motsetning til DMS-format som stykker opp koordinatene i grader, minutter og sekunder).
...
Til identifikasjon kan du rett og slett bruke din private e-postadresse. E-postadressen setter vi inn i <i>headeren</i> til forespørselen vår. Headerene er en rekke indeks-verdi-par som sendes som en del av forespørselen. E-postadressen vil legges på headeren `'User-Agent'`. I tillegg skal vi eksplisitt be om å få et JSON-objekt tilbake, i motsetning til andre formater de kunne finne på å returnere. Dette gjør vi ved å sette headeren `'Accept'` til `application/json`. Vi kommer til å spesifisere headerne ved hjelp av en Python-tabell, og det samme med koordinatene vi sender ved som parametere. Koden for å sende forespørselen blir da seende slik ut (husk å bytte ut e-postadressen om du kopierer denne koden!):
...
headers=headere, params=parametere)
```

Flott, men hvordan vet vi hva vi får i responsen? Oversikten over endepunkter inneholder et eksempel for en respons fra `/compact`-endepunktet, som er et stort JSON-objekt. Det er et objekt med et `'properties'`-felt, som igjen inneholder et felt som heter `'timeseries'` som er en liste. Denne lista inneholder værmeldinger for det gitte stedet for en rekke tidspunkter. Objektet som gir oss informasjon om været akkurat nå finner vi igjen i `['data']['instant']['details']`. Som et eksempel kan vi hente ut en temperatur og korresponderende tidspunkt fra responsen vi fikk over ved å skrive:
```python
data = respons.json()
...
print('På tidspunktet', tidspunkt, 'var temperaturen', temperatur, 'i Oslo')
```

Om alt klaffer, skal du ha fått en meldingsutskrift som ser omtrent slik ut:
```
På tidspunktet 2021-07-11T11:00:00Z var temperaturen 22.5 i Oslo
```
...
Vi rasker med oss en modul til: Vi tar en kjapp kikk på hvordan man kan lage vindubaserte programmer i Python. Dette gjør det mulig å lage programmer som er lettere og mer intuitive å bruke, i tillegg til at man kan presentere visuell informasjon. Det er flere moduler der ute som kan brukes for å lage vinduer, men i dette eksempelet skal vi bruke modulen `tkinter`. Den offisielle dokumentasjonen for `tkinter` kan du finne <a href="https://docs.python.org/3/library/tkinter.html">her</a>. Dessverre vil antakeligvis dokumentasjonen være vanskelig å lese, ettersom den benytter seg mye av <i>klasser</i>, som er et Python-konsept vi ikke har sett på i denne innføringen.

`tkinter` kommer ofte ferdig installert med Python. Dersom dette ikke er tilfellet for ditt oppsett, må du installere `tkinter` ved hjelp av `pip`, slik vi gjorde med `requests`-modulen over.

Den vanligste måten å importere `tkinter` på, er slik som dette:
```python
import tkinter as tk
```
Når vi legger på `as tk` etter modulnavnet, forteller vi Python at vi vil bruke navnet `tk` for å referere til modulen. Dette er litt kortere å skrive, og det gjør det mer behagelig å skrive programmet, for vi kommer til å referere til modulen flere ganger.


`tkinter` inneholder objekter og funksjoner for å konstruere vinduer og komponenter du kan plassere i vinduet, som tekst og knapper. For å lage et vindu bruker vi
```python
vindu = tk.Tk()
```
Legg merge til at funksjonen `Tk()` staves med stor forbokstav. I Python er dette en vanlig konvensjon for <i>konstruktører</i>, som er funksjoner som lager og returnerer <i>instanser</i> av <i>klasser</i>. Dette er konsepter som vi ikke vil gå igjennom her; for våre formål holder det å tenke på konstruktører som vanlige funksjoner som returnerer objekter.

Når vi har et `Tk`-objekt, har vi alledere et vindu vi kan vise fram på skjermen. Men kjører du programmet over, vil det avsluttes nesten umiddelbart uten at vinduet dukker opp. For å vise vinduet, må vi kalle `mainloop()`-medlemsfunksjonen på `Tk`-objektet. `mainloop()` "fanger" Python, slik at den ikke leser videre på koden før vinduet blir lukket. Så et helt fundamentalt vindusprogram kan se slik ut:
```python
import tkinter as tk
...
```python
vindu.title('Dette er en tittel!')
vindu['bg'] = 'red'
```
Husk å legge disse to linjene før du kaller `mainloop`, ellers vil de ikke kjøres før etter vinduet er lukket.



La oss legge til en knapp. Når vi lager en knapp bruker vi `Button`-funksjonen. Den tar ett argument, nemlig vinduet knappen skal vises i. Hvis vi fortsetter på programmet over, kan vi altså lage knappen slik:
```python
knapp = tk.Button(vindu)
```
Dette alene er ikke nok for at knappen skal vises i vinduet. Til dét trenger vi å spesifisere <i>hvordan</i> knappen skal plasseres. Her skal vi gjøre det ved hjelp av medlemsfunksjonen `pack`. Vi kan kalle `pack` uten argumenter slik:
```python
knapp.pack()
```
Dette vil plassere knappen, som akkurat nå bare er en liten grå firkant, midt på toppen av vinduet. Pass på at både initialiseringen av `knapp` og `pack`-kallet gjøres før kallet til `mainloop`.

Dersom vi vil være mer kreative med plasseringen av knappen, kan vi spesifisere hvilken av de fire sidene vi vil plassere knappen på ved hjelp av `side`-argumentet. Vi kan også legge inn mellomrom på sidene og over og under knappen henholdsvis med argumentene `padx` og `pady`. Verdien til `padx` og `pady` oppgis i antall piksler. For å plassere knappen til venstre, med litt mellomrom fra kanten, kan vi i stedet for kallet over bruke
```python
knapp.pack(side='left', padx='30')
```

Et tomt rektangel er ganske intetsigende. For å gjøre den mer informativ kan vi bruke `'text'`-attributtet. Vi setter dette attributtet på samme måte som vi satte bakgrunnsfarge på vinduet over:
```python
knapp['text'] = 'Dette er en knapp!'
```
Andre attributter man kan sette på knapper inkluderer bredde og høyde (`'width'` og `'height'`, oppgis i piksler) og `'fg'` og `'bg'`, som styrer henholdsvis tekstfarge og farge på resten av knappen.

Naturligvis vil vi gjerne at knappen skal gjøre mer enn å bare ligge stille og se søt ut. Hvordan bestemmer vi hva som skjer når en bruker trykker på knappen? Svaret er attributtet `'command'`, hvis verdi er en funksjon. Når brukeren trykker på knappen, kjøres funksjonen.
...
knapp['command'] = hei
```
Legg merke til at når vi setter `'command'`-attributtet, skriver vi `hei` og ikke `hei()`. Dette er fordi vi vil referere til funksjonen selv og <i>ikke</i> kalle den og referere til returverdien (som i dette tilfellet ville vært ingenting). Prøv å legge inn dette i programmet (fortsatt før `mainloop`-kallet), og sjekk at du får ut tekst hver gang du trykker på knappen!

Til slutt tar vi et litt mer sammensatt eksempel der vi bruker det vi har sett i denne seksjonen kombinert med `random`-modulen:
```python
from random import randint
import tkinter as tk
...
### Litt om det å Finne ut Hvordan man Gjør Ting™

Hvis to tok et titt på noen av dokumentasjonslenkene i denne posten, kan det hende at du allerede har mistet all lyst til å prøve å finne pakker du har lyst til å bruke, og ikke minst finne ut hvordan du bruker dem. Heldigvis er det ofte mye enklere måter å finne svar på spørsmålene du måtte ha.

Det første er rett og slett å søke etter svar på Internett. La oss ta et eksempel - i den foregånde seksjonen snakket vi ikke om hvordan vi kan endre størrelse på teksten i knappen vi lagd. Denne informasjonen er heller ikke lett tilgjengelig i dokumentasjonen. I stedet kan vi ta i bruk en søkemotor og bruke for eksempel søketeksten "tkinter button text size" og se om vi finner noen andre som har lurt på dette tidligere. I dette spesifikke eksempelet blir du kanskje ledet fram til nettsiden <a href="https://stackoverflow.com/questions/20588417/how-to-change-font-and-size-of-buttons-and-frame-in-tkinter-using-python">https://stackoverflow.com/questions/20588417/how-to-change-font-and-size-of-buttons-and-frame-in-tkinter-using-python</a>, som gir deg et eksempel på hvordan du kan sette skriftstørrelsen på knappen.

I tillegg kan det være nyttig å konsultere kunstig intelligens, men som vanlig er det lurt å ta svarene man får med en klype salt.

Det er ingen skam i å finne svar på alt mulig du måtte lure på på disse måtene, i stedet for å prøve å lete etter spesifikke svar i dokumentasjonen. Sannheten er at moduler og biblioteker er flinke på å finne opp nye konsepter og taksonomi, noe som gjør det krevende å lære seg alle moduler man støter borti; som kan være svært mange. Snarveier som disse er faktisk de vanligste metodene for å finne ut hvordan man utfører enkelte oppgaver i ukjente moduler, selv for profesjonelle programmerer.

### Å stykke opp et program i moduler
...
For å importere andre Python-filer, la oss si `kule_funksjoner.py`, trenger du at denne fila og fila det importeres inn i ligger i samme mappe. Deretter trenger du bare å skrive
```python
import kule_funksjoner
```
Legg merke til at vi bare skriver `kule_funksjoner` uten `.py` på slutten. Det er alt! Nå kan du bruke funksjonene fra `kule_funksjoner.py` i fila der du skrev linja over!

Du kan også importere filer inn i andre filer enn hovedfila også. Hvis du for eksempel har en fil som heter `hei.py` som inneholder en funksjon:
```python
def si_hei():
    print('Hei?')
```

Og en annen fil som heter `superhei.py` med innholdet
```python
import hei
...
Og ikke vær redd for å benytte en søkemotor (eller konsultere en kunstig intelligens) når du trenger det!
...
3. Lag en gjettelek med et vindu. Velg et tilfeldig tall med `random`-modulen og bruk `tkinter` for å lage et brukergrensesnitt med et tekstfelt og en knapp for å gjette, pluss tekst som forteller brukeren om de gjettet for høyt eller for lavt. Du kan bruke `tk.Entry()`-funksjonen for å lage innputtekstfelt og `tk.Text()` for å lage tekst som skal vises til brukeren.
