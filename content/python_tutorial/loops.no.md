...
Helt spesifikt skal vi gå igjennom `while`-løkker i denne posten. Det er også en annen form for
løkker, `for`-løkker, som vi kommer til å gå igjennom i en senere post.
...
En `while`-løkke er på formen
```python
while <betingelse>:
    <løkkeblokk>
```

Den starter med en linje som begynner med `while` fulgt av en betingelse, og med et kolon på slutten. Deretter følger
en innrykket blokk, akkurat som i hvis-setningen vi så på i posten om betingelser.

`while`-løkken blir kjørt som følger: Når Python kommer til linjen som begynner på `while`, sjekker den om
betingelsen er sann. Hvis den er sann, kjører Python gjennom koden i løkkeblokken. Etter at den har nådd slutten
på den innrykkede blokken, hopper den <i>tilbake</i> til `while`-linja og sjekker betingelsen på nytt.
Dersom betingelsen er sann, kjører den igjennom løkkeblokken på nytt, og fortsetter slik igjen og igjen helt
til betingelsen er usann når Python leser `while`-linja. Når dét skjer, fortsetter Python fra første
...
Først definerer vi en variabel `i` til å være 0. `i` er
en variabel vi kommer til å øke for hver gang vi går igjennom løkken,
og som vi bruker i betingelsen i `while`-linja.
Deretter kommer vi til `while`-linja, og Python sjekker
om `i` er mindre enn 5. Det er den såvisst, og vi går
inn i løkkeblokken. Her kjøres en `print()`-setning,
som gir opphav til den første linja vi ser i utputt, som
forteller oss at `i` er 0.

Til slutt i `while`-blokken er en linje vi ikke har sett
før: `i += 1`. Dette er bare en kortform for `i = i + 1`,
som altså gjør at verdien til variabelen `i` økes med 1.

Deretter fortsetter Python fra `while`-linja igjen,
men denne gangen har variabelen `i` verdien 1.

Slik fortsetter Python kjøringen av løkkeblokken. Som vi ser
i utputt, kjøres `print()`-linja hver gang
Python går igjennom løkkeblokken, og vi ser også
at `i` økes for hver iterasjon.

Den siste gangen løkkeblokken kjøres, er `i` lik 4.
...
at siste iterasjon vil bruke `i` med verdien 5,
...
Når du skriver egne `while` -løkker, er det
...
programmet. I IDLE gjøres dette med `Shell -> Restart Shell`,
eller `Ctrl+F6` (`Cmd+F6` på Mac) på tastaturet.
Om du kjører programmet i en terminal, kan du ofte bruke `Ctrl+C`.

</div>

### Vi legger en blokk inne i en blokk

I det følgende eksempelet kommer vi til å bruke `%`-operatoren, også kalt
...
Når Python kommer inn i løkka for første gang, er `i` lik
0. 0 er et partall, så `i % 2 == 0` evalueres til sant, slik
at hvis-blokken kjøres og vi får en linje ut på skjermen.
Deretter tar hvis-blokken slutt, fordi neste linje har mindre
innrykk enn `print()`-linja. Dermed kjøres `i += 1` i
hver iterasjon av løkka, uavhengig av om `i` er et partall eller
ikke: I neste iterasjon er `i` lik 1, som ikke er et partall, slik at
hvis-blokken ikke kjøres, men verdien til `i` økes til 2 likevel.

Legg merke til at vi har brukt `<=` i `while`-sjekken
for å sørge for at tallet 10 blir tatt med.


Som vi kan se over, er det ikke noe problem å kombinere `while`-løkker
...
sann. Det eneste den gjør, er å sjekke om `i` har
...
i denne koden over: `er_partall`, som vi har satt til
å være lik betingelsen `i % 2 == 0`. Vi har deretter
brukt variabelen `er_partall` direkte i hvis-setningen.
Når vi lagrer en betingelse i en variabel, får variabelen
datatypen `bool` fra engelsk <i>boolean</i>. En boolsk
variabel er enten sann - i Python `True` - eller usann - `False`.
Stor forbokstav er viktig i både `True` og `False`.
...
klart for en leser at poenget med `i % 2 == 0` i koden
...
De to kommandoene `break` og `continue` kommer oss
til unnsetning - de er
laget for å gi oss mer kontroll over hvordan løkken kjøres.

#### `break`

Når du skriver `break` på en linje inne i en løkke,
...
skrevet `True`, altså "sant" i stedet for en betingelse
i `while`-setningen. Dette fungerer helt fint,
fordi `True` i seg selv er en betingelsesverdi (en
boolsk verdi), som alltid evalueres til sann. I praksis betyr
dette at hver gang Python leser `while`-setningen for
...
å avslutte løkken. I dette tilfellet vet vi at `i` inneholder
tallet vi er ute etter, slik at vi kan skrive det til
skjerm etter løkken.

Utputt fra programmet over blir:
```
Tallet 408 er delelig på 24 og 17
```
...
men vi skal også gjøre det med `continue`.

#### `continue`

Når vi skriver `continue` inne i en løkke,
starter Python direkte på neste iterasjon, dvs. går tilbake
til `while`-linja og evaluerer betingelsen igjen, og
...
er å hoppe over iterasjonen dersom `i` er 408, slik
at ikke løkken stopper i hvis-setningen under.

Det er fristende å bare skrive `continue` alene inne
i hvis-blokken, men det vil føre til at neste iterasjon også
har `i` med verdi 408, og Python kommer til å
kjøre `continue` i all evighet til noen avslutter
programmet med makt. Når vi kjører `continue`, mister
vi nemlig resten av løkkeblokken, inkludert
linja `i += 1`, som sørger for at vi starter hver nye
iterasjon med en ny verdi i variabelen `i`. Vi må
altså sørge for at effekten av denne linja er med
når vi bruker `continue`, og legger denne linja
inn i hvis-blokken, <i>før</i> vi kjører `continue`.

`continue` kan være litt risikabelt å bruke
med `while`-løkker, som vi kan se
over. `continue` er lettere å bruke med `for`-løkker,
...
Først setter vi `i` lik 2. Dette er variabelen
...
variabel `er_primtall`, som vi har satt til å være sann.
Ideen her er at vi starter med å "gjette" at `i` er et
primtall, men hvis vi finner ut i den indre løkken at `i` ikke
kan være et primtall, setter vi denne variabelen til usann.
I tillegg lager vi variabelen `j`, som vi skal
bruke til å iterere gjennom den indre løkken. Vi skal
la `j` gå fra tallet 2 og opp til (men ikke med)
tallet `i`, og sjekke for hvert tall om det
deler `i`.

Denne delingssjekken foregår
i betingelsen til den indre hvis-setningen.
Dersom det viser seg at `j` deler `i`, kan vi med
sikkerhet si at `i` ikke kan være et primtall, og
vi setter `er_primtall` til usann og avslutter løkken.
Vær obs på at `break` inne i en dobbel løkke bare
går ut av den <i>innerste</i> løkken.

Når vi til slutt er ferdig i den indre løkken, sjekker vi
variabelen `er_primtall`. Hvis `er_primtall` er
sann, betyr det at vi ikke fant noe tall mellom 1
og `i` som deler `i`, slik at `i` må
være et primtall. Ellers må vi ha funnet et tall som
deler `i`, slik at `i` ikke kan være et primtall.

En enklere måte å forstå denne konstruksjonen på, er ved å se
på den store løkkeblokken (som tilhører den
første `while`-setningen) i isolasjon først, og
se om du skjønner hvordan den bestemmer om `i` er et
primtall.
...
tall fra og med 2 og til, men ikke med, `i`, ettersom
...
og deretter skriver en (forhåndsdefinert) setning det antallet ganger til skjermen.
